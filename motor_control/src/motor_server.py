...
def handle_move_command(req):
    move_command(req.a, req.b)
    return 'received left: '+req.a+' right: '+req.b

def move_command_server():
    rospy.init_node('move_command_server')
    s = rospy.Service('move_command', movement_command, handle_move_command)
    print("Ready to move")
    rospy.spin()
